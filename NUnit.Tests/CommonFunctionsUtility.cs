...
namespace NUnit.Tests2
{
    public class CommonFunctionsUtility
    {
        ...
        public CommonFunctionsUtility(IWebDriver driver, ExtentReports extent, ExtentTest test, List<string> listOfErrors)
        {
            this.driver = driver;
            this.extent = extent;
            this.test = test;
        ...
        public void ValidateInputForBasket(string inputId, string errorMessage)
        {
            ...
            if (inputErrors != null && inputErrors.Count > 0)
            {
                ...
                //errorMessage = $"Invalid input with ID: {inputId}";
        ...
        public void ValidateInputForAdvisor(string inputId, string errorMessage)
        {
            ...
            if (inputErrors != null && inputErrors.Count > 0)
            {
                test.Log(Status.Info, errorMessage + " for " + inputId);
                listOfErrors.Add(errorMessage);
                //errorMessage = errorMessage + " @fieldId: " + inputId;
                //throw new Exception(errorMessage);  
            }
            else
